...
def createDiagram(name, filters):
    ...
    ops_minus_by_dates = {}
    ops_plus_by_dates = {}
    for d in range((filters['dates'][1] - filters['dates'][0]).days + 1):
        ops_minus_by_dates[(filters['dates'][0] + dt.timedelta(days=d)).strftime("%d/%m/%y")] = 0
        ops_plus_by_dates[(filters['dates'][0] + dt.timedelta(days=d)).strftime("%d/%m/%y")] = 0


    for op in operations:
        if (op.user.id == filters["user"] and
            op.category.id in filters["categories"] and
            filters["dates"][0]<=op.date<=filters["dates"][1]):
            op: Operation
            if op.type:
                ops_plus_by_dates[op.date.strftime("%d/%m/%y")] += op.amount
            else:
                ops_minus_by_dates[op.date.strftime("%d/%m/%y")] += op.amount

    f, a = plt.subplots()

    names = list(ops_minus_by_dates.keys())
    values_minus = list(ops_minus_by_dates.values())
    values_plus = list(ops_plus_by_dates.values())

    x = len(names)
    wi

    fig, a = plt.subplots()
    p1 = a.bar(names, values_minus, label="Расходы")
    p2 = a.bar(names, values_plus, label="Доходы")
    a.bar_label(p1, padding=3)
    a.bar_label(p2, padding=3)
